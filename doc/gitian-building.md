...
*Setup instructions for a gitian build of NXBoost using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the NXBoost
...
- [Building NXBoost](#building-nxboost)
...
Clone the git repositories for nxboost and gitian and then checkout the nxboost version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nxboost repository
...
To build NXBoost (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the nxboost repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nxboost/.git/
...
./bin/gbuild --commit nxboost=${COMMIT} --url nxboost=${URL} ../nxboost/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nxboost=${COMMIT} --url nxboost=${URL} ../nxboost/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nxboost=${COMMIT} --url nxboost=${URL} ../nxboost/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nxboost-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nxboost-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nxboost-build.assert
```
...
[nxboost/gitian.sigs](https://github.com/nxboost-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
